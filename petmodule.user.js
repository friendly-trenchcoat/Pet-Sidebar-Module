...
(function () {
    ...
    function get_checkout(v, hunger) {
        ...
        if (!notice) {
            ...
            return is_very_hungry(hunger) ? 0 : -1;
        }
    ...
    function Neolodge() {
        ...
        // clear expired timer if pet isn't very hungry
        for (let petname in PETS) {
            if (PETS[petname].owner == USER && now > PETS[petname].neolodge && !is_very_hungry(PETS[petname].hunger)) {
    ...
    function is_very_hungry(hunger) {
        return ['dying', 'starving', 'famished', 'very hungry'].includes(hunger);
    }
